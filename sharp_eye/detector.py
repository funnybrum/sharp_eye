...
class MotionDetector(object):
        ...
        while True:
            ...
            if (motion is not False) or \
               ('debug' in config and config['debug']):
                log('Camera "%s", frame %s, motion: %s' % (config['identifier'],
                                                           self.frame_count,
    ...
    def _get_frame(self):
        ...
        frame = self.camera.snapshot_img(retries=config['motion']['camera_retry_count'])
        self.frame_count += 1
    ...
    def _save_frame(self, frame, motion_mask, motion_info):
        if 'snapshot' in config:
            cv2.imwrite(config['snapshot'], frame)

        if 'snapshot_history' in config:
            if config['full_snapshot_history']:
                # Write each frame on the drive
                cv2.imwrite('%s/ff_%04d.jpg' % (config['snapshot_history'], self.frame_count), frame)
            elif motion_info['motion_detected']:
                # Or in case of motion - save motion info on the drive
                cv2.imwrite('%s/img_%04d.jpg' % (config['snapshot_history'], self.frame_count), motion_mask)
                cv2.imwrite('%s/ff_%04d.jpg' % (config['snapshot_history'], self.frame_count), frame)
                ff_info = open('%s/ff_%04d.txt' % (config['snapshot_history'], self.frame_count), 'wb')
