...
class Camera(object):
    ...
    def snapshot(self, retries=0, retry_delay=1):
        ...
        """
        while True:
            # log('Camera %s - getting info' % config['identifier'])

            info = urllib2.urlopen(self.url + '/cgi-bin/hi3510/param.cgi?cmd=snap&',
                                   timeout=self.timeout).read()

            # log('Camera %s - info received' % config['identifier'])
            # print info
            if not info or \
               'var path="/tmpfs/snap_tmpfs/' not in info or \
               '.jpg"' not in info:
                if retries:
                    retries -= 1
                    time.sleep(retry_delay)
                    # log('Camera %s - retrying info' % config['identifier'])
                else:
                    # log('Camera %s - couldn\'t get info' % config['identifier'])
                    return None
            else:
                break

        # log('Camera %s - received info' % config['identifier'])
        snapshot_url = self.url + info.split('"')[1]
